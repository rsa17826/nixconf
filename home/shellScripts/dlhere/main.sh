...
find_best_target_dir() {
  local src="$1" fname="$2"
  mapfile -d '' -t matches < <(find "$DEST" -mindepth 1 -not -path "*/.template-base/*" -not -path "*/*.crdownload" -type f -name "$fname" -print0 2>/dev/null)
...
mapfile -d '' -t newfiles < <(find "$SRC" -maxdepth 1 -not -path "*/.template-base/*" -not -path "*/*.crdownload" -type f -mmin -0.5 -print0)
